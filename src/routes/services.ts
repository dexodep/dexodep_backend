...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const {
            name, type, server_id, github_repo, branch, root_dir,
            build_command, start_command, app_port, env_vars, domain,
        ...
        const result = await db.query(
            `INSERT INTO services (user_id, server_id, name, type, github_repo, branch, root_dir,
        ...
        if (domain && domain.trim()) {
            await db.query(
                `INSERT INTO domains (service_id, domain) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
                [service.id, domain.trim()]
            );
        }

        const webhookUrl = `${config.github.callbackUrl.replace('/api/auth/github/callback', '')}/api/webhooks/github/${service.id}`;
...
router.get('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const result = await db.query(
            `SELECT s.*, sv.name AS server_name, sv.host AS server_host
             FROM services s
             LEFT JOIN servers sv ON s.server_id = sv.id
...
router.get('/:id/deployments', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const result = await db.query(
            `SELECT id, status, phase, triggered_by, commit_sha, commit_message, duration_ms, created_at, finished_at
             FROM deployments
             WHERE service_id = $1
