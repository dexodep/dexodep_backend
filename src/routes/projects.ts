...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const result = await db.query(
            `INSERT INTO projects (user_id, server_id, name, github_repo, branch, root_dir, build_command, start_command, app_port, env_vars, domain, webhook_secret)
       ...
       RETURNING *`,
        ...
        const webhookUrl = `${config.github.callbackUrl.replace('/api/auth/github/callback', '')}/api/webhooks/github/${project.id}`;
...
router.get('/:id/deployments', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const result = await db.query(
            `SELECT id, status, triggered_by, commit_sha, commit_message, created_at
       FROM deployments
       WHERE project_id = $1
