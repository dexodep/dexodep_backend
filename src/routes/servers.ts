...
router.get('/', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const result = await db.query(
            'SELECT id, name, host, username, status, os_info, created_at FROM servers WHERE user_id = $1 ORDER BY created_at DESC',
            [req.userId]
        );
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const result = await db.query(
            `INSERT INTO servers (user_id, name, host, username, ssh_private_key)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, name, host, username, created_at`,
            [req.userId, name, host, username, ssh_private_key]
        );
...
router.get('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const result = await db.query(
            'SELECT id, name, host, username, status, os_info, created_at FROM servers WHERE id = $1 AND user_id = $2',
            [req.params.id, req.userId]
        );
...
router.post('/:id/test', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        await db.query(
            'UPDATE servers SET status = $1, os_info = $2 WHERE id = $3',
            ['active', JSON.stringify({ name: osName }), req.params.id]
        );
...
router.post('/:id/home-dir', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const result = await db.query(
            'SELECT * FROM servers WHERE id = $1 AND user_id = $2',
            [req.params.id, req.userId]
...
router.get('/:id/setup', async (req: Request, res: Response) => {
    ...
    try {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            const cmdResult = await ssh.execCommand(step.cmd, {
                ...
                onStderr(chunk: Buffer) {
                    ...
                    if (/^[\s#%\d\.kMB]+$/.test(text) || text.includes('###')) return;
                    text.split('\n').forEach((line: string) => {
                        const t = line.trim();
                        if (t && t.length > 2 && !t.match(/^[#%\s]+$/)) send('log', { line: `[stderr] ${t}` });
                    });
                },
            });
        ...
        await db.query(
            'UPDATE servers SET status = $1, os_info = $2 WHERE id = $3',
            ['active', JSON.stringify({ name: 'Ubuntu', setup: 'complete' }), req.params.id]
        );
...
router.get('/:id/commits', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        const result = await db.query(
            'SELECT * FROM servers WHERE id = $1 AND user_id = $2',
            [req.params.id, req.userId]
        ...
        const servicesResult = await db.query(
            'SELECT root_dir FROM services WHERE server_id = $1',
            [req.params.id]
        );
        ...
        for (const service of servicesResult.rows) {
            try {
                const gitLog = await ssh.execCommand(
                    `cd ${service.root_dir} && git log --oneline -10 --pretty=format:'{"hash":"%H","message":"%s","author":"%an","date":"%ai"}'`
                );
    ...
    try {
        ...
        try {
            const journalResult = await ssh.execCommand(
                `journalctl --since "${since}" --no-pager --output=short-iso 2>/dev/null | tail -200`
            );
        ...
        if (logs.length === 0) {
            const fallback = await ssh.execCommand(
                `tail -200 /var/log/syslog 2>/dev/null || tail -200 /var/log/messages 2>/dev/null || echo "No logs available"`
            );
