...
exports.deploymentEvents = void 0;
exports.runDeployment = runDeployment;
const events_1 = require("events");
const node_ssh_1 = require("node-ssh");
const db_1 = __importDefault(require("../db"));
const runtime_1 = require("./runtime");
exports.deploymentEvents = new events_1.EventEmitter();
exports.deploymentEvents.setMaxListeners(100);
function emitLog(deploymentId, line) {
    exports.deploymentEvents.emit(`log:${deploymentId}`, { type: 'log', line });
}
function emitPhase(deploymentId, phase) {
    exports.deploymentEvents.emit(`log:${deploymentId}`, { type: 'phase', phase });
}
function emitStatus(deploymentId, status) {
    exports.deploymentEvents.emit(`log:${deploymentId}`, { type: 'status', status });
}
async function runCmd(ssh, command, deploymentId, allLogs, allowFail = false, silent = false) {
    if (!silent) {
    ...
    try {
        ...
        const result = await ssh.execCommand(wrappedCmd, {
            ...
            onStderr(chunk) {
                ...
                if (/^[\s#%\d\.kMB]+$/.test(text) || text.includes('###') || /^\s*\d+\.\d+%/.test(text.trim()))
                    return;
...
async function setPhase(deploymentId, phase, allLogs) {
    await db_1.default.query('UPDATE deployments SET phase = $1 WHERE id = $2', [phase, deploymentId]);
    const header = `\n=== ${phase} ===`;
    emitLog(deploymentId, header);
    emitPhase(deploymentId, phase);
    allLogs.push(header);
}
async function runDeployment(deploymentId, serviceId, commitSha, commitMessage) {
    const allLogs = [];
    let ssh = null;
    const startTime = Date.now();
    try {
        const svcResult = await db_1.default.query(`SELECT s.*, sv.host AS server_host, sv.username AS server_username, sv.ssh_private_key AS server_ssh_private_key,
        ...
        const useSystemd = (0, runtime_1.usesSystemd)(svc.runtime);
        await db_1.default.query(`UPDATE deployments SET status = 'running', commit_sha = $2, commit_message = $3 WHERE id = $1`, [deploymentId, commitSha || null, commitMessage || null]);
        await db_1.default.query(`UPDATE services SET status = 'deploying' WHERE id = $1`, [serviceId]);
        ...
        const installSteps = (0, runtime_1.getRuntimeInstallSteps)(svc.runtime);
        for (const step of installSteps) {
            ...
            if (check.success) {
                ...
                if (!install.success)
                    throw new Error(`Failed to install ${step.label}`);
        ...
        if (useSystemd) {
            ...
            const unit = (0, runtime_1.generateSystemdUnit)(pm2Name, svc.root_dir, svc.start_command, svc.app_port, svc.env_vars || {});
            ...
            if (pm2Check.success) {
                ...
                if (svc.runtime === 'ruby')
                    interpreter = '--interpreter ruby';
                const startResult = await runCmd(ssh, `cd ${svc.root_dir} && PORT=${svc.app_port} pm2 start ${svc.start_command} --name ${pm2Name} ${interpreter}`, deploymentId, allLogs);
        ...
        if (svc.domain && svc.domain.trim()) {
            ...
            const nginxConfig = (0, runtime_1.generateNginxConfig)(svc.domain, svc.app_port);
            ...
            const sslResult = await runCmd(ssh, `sudo certbot --nginx -d ${svc.domain} --non-interactive --agree-tos -m admin@${svc.domain}`, deploymentId, allLogs, true);
            if (sslResult.success) {
                emitLog(deploymentId, '✓ SSL certificate installed');
                allLogs.push('✓ SSL certificate installed');
                await db_1.default.query(`UPDATE domains SET ssl_status = 'active' WHERE service_id = $1 AND domain = $2`, [serviceId, svc.domain]);
            }
            else {
        ...
        const healthCheck = await runCmd(ssh, `curl -sf -o /dev/null -w "%{http_code}" http://localhost:${svc.app_port}${healthPath} || echo "UNREACHABLE"`, deploymentId, allLogs, true);
        ...
        allLogs.push(`\n✅ Deployment completed in ${Math.round(durationMs / 1000)}s!`);
        await db_1.default.query(`UPDATE deployments SET status = 'success', logs = $2, phase = 'completed', duration_ms = $3, finished_at = NOW() WHERE id = $1`, [deploymentId, allLogs.join('\n'), durationMs]);
        await db_1.default.query(`UPDATE services SET status = 'live', updated_at = NOW() WHERE id = $1`, [serviceId]);
        ...
        emitLog(deploymentId, errMsg);
        allLogs.push(errMsg);
        await db_1.default.query(`UPDATE deployments SET status = 'failed', logs = $2, phase = 'failed', duration_ms = $3, finished_at = NOW() WHERE id = $1`, [deploymentId, allLogs.join('\n'), durationMs]);
        await db_1.default.query(`UPDATE services SET status = 'failed', updated_at = NOW() WHERE id = $1`, [serviceId]);
